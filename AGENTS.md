...
candor-java — the family's reference engine (spec 0.10) — reports, for every method in a JVM codebase
...
1. **Tell the user which version they're on.** Run the offline version flag and state it plainly —
   e.g. *"This project is on candor-java <version> (spec 0.10)."*
   ```sh
...
while `.candor.spec` is the contract version, `0.10`.)
