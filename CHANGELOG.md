...
- No engine change. `jbang-catalog.json` pins `v0.38.2`, verified by RESOLVING it (HTTP 200) rather
  than by reading the string — the 0.24 failure was a pin naming a URL that 404'd. Released to keep the
  family line aligned at 0.38.2.
...
- **`show` and `map` returned the WARNING INSTEAD OF THE ANSWER — the ⟨0.32⟩ descriptive hedge, ruled
  ...
  **CONTROLS, WRITTEN FIRST AND BOTH DIRECTIONS ASSERTED**, because the safe-looking empty value passes
  ...
  GATES: ./gradlew test (834 tests, 0 failed); test/smoke.sh (546 passed, 0 failed). Conformance was
...
- ⚠ **A class that did nothing but READ HTTP HEADERS passed `deny Net` at exit 0 with an EMPTY function
  ...
  **Corpus A/B, 933 jars** (the Gradle module cache + an exploded production war) under `deny Net`,
  ...
  defect bites the project whose ONLY Net is a header read, which is exactly the fixture above. the field case's
...
- ⚠ **`--parallel` swallowed the refusal the single route produces, and `gate --report` went GREEN over
  ...
  **gate 0 → 2 under `deny Exec`, `deny Net` and `deny Fs`; a 24-artifact control sample and the field case's repo
...
- ⚠ **`deny Exec` did not see a subprocess being ASSEMBLED — only one being launched.** This engine
  ...
  **Measured for over-charge, which is the half that decides it.** JVM corpus A/B, 933 jars (the Gradle
  ...
  methods, and every one of those 9 launches in the same body. the field case's `build/classes/java/main`:
...
### Added — the peek now reads multi-release overrides

A multi-release jar ships version-specific classes under `META-INF/versions/<N>/`. candor analysed the
BASE class and skipped the override, so **an effect present only in the versioned copy — the copy a JVM
of that version actually runs — sat outside every verdict.** Measured on `log4j-api:2.23.1`: scanning the
base copies against base-with-overrides-applied gave **21 functions a materially different verdict** and
left **5 that exist only in the versioned copy** unjudged. The peek now reads them through the same
parse and classifier, and under ⟨0.30⟩ a denied effect found there moves the verdict. A jar whose
override is clean stays green.

### Fixed — a dependency's disclosed uncertainty reaches its callers

A chained dependency entry whose ONLY signal was `incomplete` (no effects) was dropped before its marker
could propagate, so a caller read certainly-nothing about a callee that had declared it could not be
fully seen. This engine's own report writer began emitting that shape at 0.29.1, so our own reports held
entries the chaining path could not carry.
...
- **An unreadable multi-release override claimed to have been read.** The "did anything fail to parse"
  check ran after the "is there anything here" check, so a jar whose versioned copies ALL fail to parse
  was indistinguishable from one with none — and the class then published `peeked: true` beside an empty
  `outOfScope`, which is the sentence *I read every override and none performs a denied effect* over an
  override nobody read. Measured with a versioned copy at bytecode major 99 performing `Exec`: exit 0.
...
- **`jbang-catalog.json` names v0.29.0.** Its DESCRIPTION carries the current contract
  (candor-spec 0.29), and its `script-ref` moved only once the release existed — the URL was
  RESOLVED first (HTTP 200, 1141969 bytes, matching the release asset), because a pin naming a URL
  is not the URL existing, which is the 404 `release-preflight [3]` exists to prevent.
...
- **jbang catalog pinned to v0.28.2** — it names a published artifact, so it moves after the release.
