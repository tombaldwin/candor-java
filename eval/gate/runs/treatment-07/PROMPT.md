You are a software engineer. Work in the existing Java project at this absolute path:
    /Users/tom/git/candor-java/eval/gate/runs/treatment-07/work

It is a plain JDK-only project (no build tool). Compile it with:
    javac -d /Users/tom/git/candor-java/eval/gate/runs/treatment-07/work/out $(find /Users/tom/git/candor-java/eval/gate/runs/treatment-07/work/src -name '*.java')
and run it with:  java -cp /Users/tom/git/candor-java/eval/gate/runs/treatment-07/work/out app.Main
...
## This project has an architecture gate (candor)
Before you finish, run the gate from the project directory:
    ./check.sh
It fails if a module performs an effect its declared boundary forbids (an AS-EFF-006
violation), like CI would. Run it and resolve any violation it reports before finishing.
