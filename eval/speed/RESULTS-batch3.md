...
- **`<a model class with no path to the target>`** — two control agents included it; it has no path
  ...
  excluded it.
- **`<the abstract scheduled-task base's execute>`** — one control agent included it, and it is **right**: the
  abstract `execute` calls the abstract `performTask()` (<the abstract base's source>), which
  dispatches to the Net-performing subclass overrides (`<a Net-performing subclass>`/`<a Net-performing subclass>...`). **candor
...
- The `<the abstract scheduled-task base's execute>` miss is a genuine candor-java limitation, reported here rather
