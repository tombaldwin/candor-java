...
public class Candor {
     ...
     *  lead a minor rung (candor-spec §"Versioning policy" — the version ladder): it declares `0.12` (the
     *  0.12 tier-2 rung promotes the gains `origin` field — existing|new|unknown, keyed on the baseline
     *  callgraph sidecars — into the pinned contract) while a sibling may remain on an earlier floor and
     *  raise as it implements it. Additive-only, so an older consumer is unaffected. */
    static final String SPEC_VERSION = "0.12";
