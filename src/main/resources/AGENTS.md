...
1. **Tell the user which version they're on.** Run the offline version flag and state it plainly —
   e.g. *"This project is on candor-java 0.8.4 (spec 0.8)."*
   ```sh
...
2. **Check whether it's current.** candor-java never phones home — it analyzes for the `Net` effect
   ...
   curl -s -H 'User-Agent: candor-version-check' \
     ...
     | grep -o '"tag_name": *"[^"]*"'                                    # latest -> "tag_name":"v0.8.4" (compact)
   ```
3. **If it's behind, *ask* before upgrading.** Say e.g. *"candor-java 0.8.5 is available (you're on
   0.8.4) — upgrade before I scan?"* and run `jbang --fresh candor@tombaldwin/candor-java` only if the
...
while `.candor.spec` is the contract version, `0.8`.)
