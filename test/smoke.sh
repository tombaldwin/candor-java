...
want "envelope declares the spec contract 0.13" "$rep" '"spec": "0.13"'
...
# must NOT be (precision — the field case has such a method and it stays a normal method).
cat > "$W/src/Tk.java" <<'J'
...
want "--help states the spec contract"             "$hp" 'candor-spec 0.13'
...
want    "--version prints the release + spec"      "$vv" '(spec 0.13)'
...
want    "README pins the spec floor (spec 0.13)"                  "$README_TXT" "spec 0.13"
...
want    "AGENTS pins the spec floor (spec 0.13)"                  "$AGENTS_TXT" "spec 0.13"
